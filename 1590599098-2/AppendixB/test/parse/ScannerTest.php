...
class test_parse_ScannerTest extends test_testutil_BaseTest {
    ...
    public function testPants() {
        $tokenizer = new gi_parse_Scanner( 'pants pants pants' );
        while ( $tok = $tokenizer->nextToken()
                != gi_parse_Scanner::EOF ) {
            $val = $tokenizer->token();
            $this->say( $val ); 
        }
    }
 ...
 */
    public function testToken() {
        ...
        $tokenizer = new gi_parse_Scanner( $str );
        $tok = $tokenizer->nextToken();  // this
        $val = $tokenizer->token();
        self::AssertEquals( $tok, gi_parse_Scanner::WORD );
/*
        self::AssertEquals( $val, 'this', "got $val" );
        $tok = $tokenizer->nextToken(); // EOL
        self::AssertEquals( $tok, gi_parse_Scanner::EOL );
        $tok = $tokenizer->nextToken(); // is
        $tok = $tokenizer->nextToken(); // SPACE
        self::AssertEquals( $tok, gi_parse_Scanner::WHITESPACE );
        ...
        $val = $tokenizer->token();
        self::AssertEquals( $tok, gi_parse_Scanner::CHAR );
        self::AssertEquals( $val, "<" );
        while ( ($tok = $tokenizer->nextToken()) 
                != gi_parse_Scanner::EOF ) {
